...
try {
  ...
  const dashboardLegendPlacement=await cdp.evaluate(`(()=>{const panels=[...document.querySelectorAll('.rd-dashboard-top > .rd-panel')];return panels.map(panel=>{const header=panel.querySelector('.rd-panel-title'),title=panel.querySelector('.rd-widget-title'),legend=panel.querySelector('.rd-dashboard-legend'),headerRect=header.getBoundingClientRect(),titleRect=title.getBoundingClientRect(),legendRect=legend.getBoundingClientRect();return {inHeader:legend.parentElement===header,rightGap:Math.round(headerRect.right-legendRect.right),titleCenterGap:Math.round((titleRect.top+titleRect.height/2)-(headerRect.top+headerRect.height/2)),legendCenterGap:Math.round((legendRect.top+legendRect.height/2)-(headerRect.top+headerRect.height/2))}})})()`);
  assert.ok(dashboardLegendPlacement.every(item=>item.inHeader&&item.rightGap<=18&&Math.abs(item.titleCenterGap)<=1&&Math.abs(item.legendCenterGap)<=1),JSON.stringify(dashboardLegendPlacement));
  ...
  assert.ok(widgetHeaderHeights.length===6&&widgetHeaderHeights.every(height=>height>=49&&height<=51),JSON.stringify(widgetHeaderHeights));
  ...
  assert.ok(compactWidgetHeaders.heights.every(height=>height>=49&&height<=51),JSON.stringify(compactWidgetHeaders));
  ...
  assert.equal(periodDetails.display,'flex',JSON.stringify(periodDetails));
  assert.equal(periodDetails.segments,4,JSON.stringify(periodDetails));
  assert.ok(periodDetails.width>0&&periodDetails.width<=100.01,JSON.stringify(periodDetails));
  ...
  const dashboardTypography=await cdp.evaluate(`(()=>{const title=document.querySelector('.rd-dashboard-card-title span:first-child'),total=document.querySelector('.rd-dashboard-card-total'),head=document.querySelector('.rd-dashboard-breakdown-head'),style=node=>{const value=getComputedStyle(node);return {size:value.fontSize,weight:value.fontWeight,color:value.color,family:value.fontFamily,lineHeight:value.lineHeight}},references=[document.querySelector('#rd-engagement-title'),document.querySelector('.rd-dashboard-top .rd-widget-title'),title];return {titleSize:getComputedStyle(title).fontSize,totalSize:getComputedStyle(total).fontSize,titleWeight:getComputedStyle(title).fontWeight,totalWeight:getComputedStyle(total).fontWeight,styles:references.map(style),breakdownLeft:head.getBoundingClientRect().left,cardsLeft:document.querySelector('#rd-dashboard-cards').getBoundingClientRect().left}})()`);
  ...
  assert.ok(dashboardTypography.styles.every(style=>JSON.stringify(style)===JSON.stringify(dashboardTypography.styles[0])),JSON.stringify(dashboardTypography));
  ...
  const settingsChrome=await cdp.evaluate(`(()=>{const remove=document.querySelector('.rd-icon-action'),help=document.querySelector('[aria-label="Как работает персональный сигнал"]'),row=document.querySelector('.rd-analytics-rule-row'),token=document.querySelector('#rd-gitlab-token'),checkbox=document.querySelector('#rd-hide-inactive-days'),settings=document.querySelector('.rd-analytics-settings');return {removeBorder:getComputedStyle(remove).borderTopWidth,saveCount:document.querySelectorAll('#rd-save-analytics').length,analyticsColumns:getComputedStyle(row).gridTemplateColumns.split(' ').length,tokenMasked:token.value.length>0&&token.dataset.saved==='true',helpVisible:getComputedStyle(help.closest('.rd-personal-signal-setting')).display!=='none',thresholdVisible:getComputedStyle(document.querySelector('#rd-signal-threshold').closest('.rd-personal-signal-setting')).display!=='none',checkboxVisible:getComputedStyle(checkbox).display!=='none',settingsHeight:settings.getBoundingClientRect().height}})()`);
  ...
  assert.ok(settingsChrome.settingsHeight<=72,JSON.stringify(settingsChrome));
